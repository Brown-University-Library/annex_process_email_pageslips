...
class ItemListMaker( object ):
    ...
    def make_item_list( self, text ):
        ...
        for ( i, line ) in enumerate( unicode_lines ):
            ...
            if self.check_start( line ) == True:
                ...
                self.item = []  # clear item
            self.conditionally_append_line_to_item( line )
            self.two_lines_past = '' if i < 2 else unicode_lines[ i - 2 ]
            self.last_line = '' if i == 0 else unicode_lines[ i - 1 ]
            self.next_line = '' if i + 1 >= len(unicode_lines) else unicode_lines[ i + 1 ]
            log.debug( 'two_lines_past, ```%s```; last_line, ```%s```; cur_line, ```%s```; next_line, ```%s```' % (self.two_lines_past, self.last_line, line, self.next_line) )
        self.items.append( self.item )  # adds last item
        self.clean_items()
        return self.items
    ...
    def check_start( self, line ):
        ...
        if line in ['Brown University', 'Brown University Library' ]:
            if 'Gateway Services, Rockefeller Library' in self.next_line.strip():
                return_val = True
            ...
            return_val = True
        log.debug( 'is-start, `%s`' % return_val )
        return return_val
